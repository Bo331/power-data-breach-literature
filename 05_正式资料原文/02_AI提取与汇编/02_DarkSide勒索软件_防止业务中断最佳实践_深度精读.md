...
```mermaid
graph TD
    A[发现主机受感染或文件变化异常] --> B[切断网络连通性]
    B --> B1[立刻拔掉物理网线并禁用所有无线/蓝牙适配器]
    B --> B2[严禁关闭受害者主机电源以留存物理内存中的解密密钥\n防止内存取证失效]
    
    A --> C[网络全局收缩隔离]
    C --> C1[立即断开全网共享盘、映射盘、网络存储卷的连接]
    C --> C2[将尚未表现出加密特征、但同网段的主机执行紧急逻辑隔离]
    
    A --> D[隔离备份介质安全恢复]
    D --> D1[物理拆除备份服务器上的联网线路]
    D --> D2[使用经脱机升级的杀毒签名对备份介质执行全面离线扫描]
    D --> D3[结合预配置系统黄金镜像Golden Image逐步离线重构受灾系统]
